...
class _TransactionState extends State<Transaction> {
          ...
          Row(
            ...
            children: [
              ElevatedButton(onPressed: () async {
                print(id);
                var result = await _myservices.deleteDataTransactionService(id);
                print("entry deleted !!!");
                getHistrytransaction();
  ...
  Widget build(BuildContext context) {
    return Scaffold( backgroundColor: Color(0xff010A43),
appBar:  AppBar(
...
body: ListView.builder(
  ...
  itemBuilder: ((context, index){
    if( _transactionHistoryList[index].type.toString() == "Expense"){
       abcd = Icon(Icons.currency_rupee,color: Color.fromARGB(255, 220, 23, 56),);
    }else{
       abcd = Icon(Icons.currency_rupee,color: Colors.green,size: 30,);
    ...
    return Card(
      elevation: 2,
        shape: RoundedRectangleBorder(
          side: BorderSide(
            color:Color.fromARGB(255, 61, 92, 118)
          ),
      ...
      child: ListTile(
        ...
        subtitle: Text(_transactionHistoryList[index].title!, style: TextStyle(fontSize: 20,color: Color.fromARGB(255, 241, 237, 225)),),
        trailing: Text(_transactionHistoryList[index].type!,style: TextStyle(fontSize: 20,color: Color.fromARGB(255, 241, 237, 225) ),),
        onLongPress: () {
          _deleteTransactionEntry(context, _transactionHistoryList[index].id);
